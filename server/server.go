...
type Server struct {
	...
	StartTime  time.Time
	CurrentIP  string
	CurrentSource string // 新增：当前 IP 来源
...
// handleGetConfig 获取配置（敏感字段已脱敏）
func (s *Server) handleGetConfig(c echo.Context) error {
	// 获取当前配置副本
	safeCfg := s.Config.Get()
	
	// 脱敏敏感字段
	safeCfg.Server.APIKey = "***"
	
	// 脱敏 Cloudflare Token
	for i := range safeCfg.CloudflareAccounts {
		safeCfg.CloudflareAccounts[i].APIToken = "***"
	}
	
	// 脱敏 IP 提供者配置
	for i := range safeCfg.IPProviders {
		if safeCfg.IPProviders[i].Properties == nil {
			safeCfg.IPProviders[i].Properties = make(map[string]string)
		}
		props := safeCfg.IPProviders[i].Properties
		if props["password"] != "" {
			props["password"] = "***"
		}
		if props["key"] != "" {
		...
		safeCfg.IPProviders[i].Properties = props
	}
	
	return c.JSON(http.StatusOK, safeCfg)
}
...
func (s *Server) handleUpdateConfig(c echo.Context) error {
	...
	// 如果 API Key 是脱敏值，保留原值
	if newCfg.Server.APIKey == "***" || newCfg.Server.APIKey == "" {
		newCfg.Server.APIKey = currentCfg.Server.APIKey
	}
	
	// 处理 Cloudflare 账户脱敏回填
	// 策略：唯一依赖【账户名称】进行匹配。
	// 这避免了索引不一致导致的问题。如果用户改了名字，则必须重新输入 Token。
	for i := range newCfg.CloudflareAccounts {
		// 如果 Token 为空或脱敏值，尝试恢复
		if newCfg.CloudflareAccounts[i].APIToken == "***" || newCfg.CloudflareAccounts[i].APIToken == "" {
			tokenRecovered := false
			
			// 按名称查找旧配置
			for _, oldAcc := range currentCfg.CloudflareAccounts {
				if oldAcc.Name == newCfg.CloudflareAccounts[i].Name {
					// 找到了同名账户，恢复 Token
					newCfg.CloudflareAccounts[i].APIToken = oldAcc.APIToken
					tokenRecovered = true
					break
				}
			}
			
			// 如果恢复失败（说明是新账户或改了名），则必须提供 Token
			if !tokenRecovered {
				return c.JSON(http.StatusBadRequest, map[string]string{
					"error": fmt.Sprintf("账户 '%s' 未提供 API Token (重命名或新账户必须重新输入 Token)", newCfg.CloudflareAccounts[i].Name),
				})
			}
		}
	}
	...
	// 处理 IP 提供者脱敏回填
	for i := range newCfg.IPProviders {
		newProps := newCfg.IPProviders[i].Properties
		// 策略：同类型第一个匹配（简化策略）
		if newProps["password"] == "***" || newProps["key"] == "*** (已配置)" || newProps["key"] == "***" {
			for _, oldProv := range currentCfg.IPProviders {
				if oldProv.Type == newCfg.IPProviders[i].Type {
					if newProps["password"] == "***" {
						newProps["password"] = oldProv.Properties["password"]
					}
					if newProps["key"] == "*** (已配置)" || newProps["key"] == "***" {
						newProps["key"] = oldProv.Properties["key"]
					}
					break 
				}
			}
		}
		newCfg.IPProviders[i].Properties = newProps
	}
	...
	go func() {
		...
		if ip != "" && s.DNSUpdater != nil {
			fmt.Println("Configuration changed, triggering immediate DNS sync...")
			// 强制同步（即使 IP 没变也检查记录是否存在）
			if err := s.DNSUpdater.UpdateIP(ip); err != nil {
				fmt.Printf("Immediate DNS sync failed: %v\n", err)
				s.DB.AddErrorLog("error", "Post-config DNS sync failed: "+err.Error())
			} else {
...
// handleExportConfig 导出配置文件为 YAML
func (s *Server) handleExportConfig(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "读取配置文件失败: " + err.Error()})
	}
...
func (s *Server) handleGetStatus(c echo.Context) error {
	...
	for _, p := range cfg.IPProviders {
		if p.Enabled {
			...
			// 检查当前 source 是否仍有效（对应的 provider 是否启用）
			if strings.EqualFold(source, p.Type) {
				sourceValid = true
			}
		}
	}
	
	// 如果当前 source 无效（对应 provider 已禁用），使用第一个启用的
	if !sourceValid && firstEnabledSource != "" {
		source = firstEnabledSource
	} else if source == "" && firstEnabledSource != "" {
		source = firstEnabledSource
	}
	...
	// 获取最后一次检查时间（使用最新记录或当前时间）
	lastUpdated := time.Now()
	if len(ipHistory) > 0 {
		lastUpdated = ipHistory[0].Timestamp
	}
...
// handleTriggerDNSUpdate 手动触发 DNS 同步
