...
import { Save, Plus, Trash2, RefreshCw, Shield, Globe, Cloud, ChevronDown, ChevronUp, Settings, Check } from 'lucide-react';
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <InputGroup label="API Token">
          <StyledInput
            type="password"
            value={account.api_token === '***' ? '' : account.api_token}
            onChange={e => onChange({ ...account, api_token: e.target.value })}
            placeholder={isZh ? "已保存 (留空保持不变)" : "Saved (Leave empty to keep)"}
          />
        </InputGroup>
      </div>
...
const ConfigPage = () => {
  ...
  const [loading, setLoading] = useState<string | null>(null); // Track specific section loading
  ...
  const handleSave = async (section: string) => {
    ...
    try {
      ...
      let msg = isZh ? '配置保存成功' : 'Configuration saved';
      if (section === 'dns') msg = isZh ? 'DNS 配置已保存并触发强制更新' : 'DNS config saved & update triggered';
      if (section === 'ip') msg = isZh ? 'IP 配置已保存' : 'IP config saved';
  ...
  const ApplyButton = ({ section }: { section: string }) => (
    <button
      onClick={() => handleSave(section)}
      disabled={!!loading}
      className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${loading === section ? 'bg-primary/50 cursor-not-allowed' : 'bg-primary hover:opacity-90'} text-primary-foreground shadow-sm`}
    >
      {loading === section ? <RefreshCw className="animate-spin" size={12} /> : <Save size={12} />}
      {isZh ? '应用更改' : 'APPLY'}
    </button>
  );
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20">
      ...
      >
        ...
        <div className="flex gap-3 w-full sm:w-auto">
          <motion.button
            ...
            className="flex-1 sm:flex-none px-4 py-2 bg-surface hover:bg-surface-hover text-content rounded-lg text-sm font-bold transition-colors shadow-sm"
          >
            {isZh ? '导出' : 'EXPORT'}
          </motion.button>
        </div>
      ...
      >
        <SectionHeader
          ...
          action={<ApplyButton section="general" />}
        />
